...
class AttendanceServiceDB:
            ...
            if schedule.startDate:
                ...
                if start > today:
                    ...
                    if target_date < start:
                        target_date = start + timedelta(days=(target_day - start.weekday() + 7) % 7)
        ...
        try:
            ...
            for s in schedules:
                result.append({
                    ...
                    "is_recurring": s.isRecurring if s.isRecurring is not None else True,
                    "location": s.location,
                    "capacity": s.maxSlots,
    ...
    def register_public_attendance(self, data):
        ...
        try:
            ...
            for participant_id, status in attendances.items():
                participant = Participant.query.filter_by(external_id=str(participant_id)).first()
                if not participant:
                    continue
            ...
            return success_response(
                ...
                data={"total": count, "date": attendance_date, "schedule_id": schedule_id}
            )
        except Exception as e:
            db.session.rollback()
            return error_response(f"Error interno: {str(e)}")
    ...
    def get_programs(self):
        ...
        try:
            ...
            return success_response(
                msg="Programas obtenidos correctamente",
                data=result
            )
        except Exception as e:
            return error_response(f"Error interno: {str(e)}")
    ...
    def get_session_detail(self, schedule_id, attendance_date):
        ...
        try:
            ...
            absent_count = len([r for r in records if r.get("status") in ["ABSENT", "JUSTIFIED"]])
